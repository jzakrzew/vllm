...
def init_nvfp4_linear_kernel() -> NvFp4LinearKernel:
    ...
    if envs.VLLM_BATCH_INVARIANT:
        bi_supported, _ = BatchInvariantNvFp4LinearKernel.is_supported()
        if bi_supported:
            force_kernel = BatchInvariantNvFp4LinearKernel
        else:
            logger.info_once(
                "VLLM_BATCH_INVARIANT is set but batch-invariant NVFP4 "
                "kernel requires Blackwell (sm100+); falling back to "
