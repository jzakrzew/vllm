...
def test_linear_batch_invariant_nvfp4_batch_size_invariance(
    ...
    global_scale = _global_scale(x_batch.reshape(-1, in_features))
    alpha = (1.0 / (global_scale * weight_global_scale)).to(torch.float32)

    out_single = linear_batch_invariant_nvfp4(
        input=x_single,
        weight=weight_fp4,
        weight_scale=weight_scale,
        input_global_scale_inv=global_scale,
        alpha=alpha,
